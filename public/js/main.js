document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dbTabBtns.length > 0) {
    ...
    if (activeTab) {
      const targetBtn = document.querySelector(`.db-tab-btn[data-tab="${activeTab}"]`);
      if (targetBtn) {
        targetBtn.click();
      }
    }
  ...
  toggleButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (isHidden) {
        ...
        btn.innerHTML = '<i class="fa-solid fa-clock-rotate-left"></i> Hide Plan';
        ...
        const savedHours = localStorage.getItem(`cc_study_hours_${playlistTitle}`);
        if (savedHours) {
  ...
  function calculateStudyPlan(panel) {
    ...
    estFinishEl.textContent = targetDate.toLocaleDateString('en-US', dateOptions);

    // Generate custom milestones based on the title
    const cleanTitle = title.replace(/Tutorial|Course|for Beginners|Beginners/gi, '').trim();
    milestoneList.innerHTML = '';

    let milestones = [];
    if (totalDays === 1) {
      milestones.push(`Watch the complete <strong>${cleanTitle}</strong> lectures. Code along to establish primary muscle memory.`);
    } else if (totalDays === 2) {
      milestones.push(`<strong>Day 1</strong>: Watch the first half. Focus on installation, tool config, and basic code patterns.`);
      milestones.push(`<strong>Day 2</strong>: Finish the course. Tackle advanced modules and build a quick sandbox app to test ideas.`);
    } else if (totalDays === 3) {
      milestones.push(`<strong>Day 1</strong>: Set up the workspace. Complete fundamental lessons and setup scripts.`);
      milestones.push(`<strong>Day 2</strong>: Work on intermediate techniques, logic flows, and code exercises.`);
      milestones.push(`<strong>Day 3</strong>: Complete advanced features, debugging patterns, and outline a capstone module.`);
    } else if (totalDays === 4) {
      milestones.push(`<strong>Day 1</strong>: Lay basic groundwork and introductory setup.`);
      milestones.push(`<strong>Day 2</strong>: Complete core syntax lessons and small practice programs.`);
      milestones.push(`<strong>Day 3</strong>: Explore intermediate structures, secondary APIs, and coding loops.`);
      milestones.push(`<strong>Day 4</strong>: Study performance reviews, watch remaining modules, and finalize portfolio code.`);
    } else {
      const part = Math.floor(totalDays / 3);
      milestones.push(`<strong>Days 1 to ${part}</strong>: Foundations. Learn concepts, basic structures, and complete configuration.`);
      milestones.push(`<strong>Days ${part + 1} to ${totalDays - part}</strong>: Build phase. Follow coding demonstrations and write exercise logic.`);
      milestones.push(`<strong>Days ${totalDays - part + 1} to ${totalDays}</strong>: Master phase. Tackle optimization, project finalization, and review docs.`);
    }
  ...
  savePlanBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const toggleBtn = document.querySelector(`.toggle-planner-btn[data-playlist-id="${playlistId}"]`);
      if (toggleBtn) {
  ...
  allPlanners.forEach(panel => {
    ...
    if (savedHours) {
      ...
      if (slider) {
        ...
        // Recalculate based on saved hours
        calculateStudyPlan(panel);
        
        // Mark the plan button as active
        const playlistId = panel.id.replace('planner-', '');
        const toggleBtn = document.querySelector(`.toggle-planner-btn[data-playlist-id="${playlistId}"]`);
        if (toggleBtn) {
